...
def generatekey2(input_path: Path):
    keys = [
        # R9s/A57t
        [
            "V1.4.17/1.4.27",
    ...
    for dkey in keys:
        ...
        pagesize, data = extract_xml(input_path, key, iv)
        if pagesize != 0:
            return pagesize, key, iv, data
    return 0, None, None, None


def extract_xml(input_path: Path, key, iv):
    filesize = input_path.stat().st_size
    with input_path.open("rb") as rf:
        pagesize = 0
...
def copy(input_path: Path, wfilename, extract_dir: Path, start, length, checksums):
    print(f"\nExtracting {wfilename}")
    with input_path.open("rb") as rf, (extract_dir / wfilename).open("wb") as wf:
        rf.seek(start)
        data = rf.read(length)
        wf.write(data)

    checkhashfile(extract_dir / wfilename, checksums, True)
...
def decryptfile(
    ...
    checkhashfile(extract_dir / wfilename, checksums, False)


def checkhashfile(path: Path, checksums, iscopy):
...
def main(input_path, extract_dir):
    ...
    if pk:
        ...
        with zipfile.ZipFile(input_path) as file:
            for zfile in file.namelist():
                print("Extracting {zfile} to {extract_dir}")
                file.extract(zfile, pwd=zippw, path=extract_dir)
            print("Files extracted to {extract_dir}")
            exit(0)

    # key,iv=generatekey1()
    pagesize, key, iv, data = generatekey2(input_path)
    if pagesize == 0:
    ...
    file_handle = (extract_dir / "ProFile.xml").open(mode="w")
    ...
    for child in root:
        for item in child:
            ...
            if child.tag in ["DigestsToSign", "ChainedTableOfDigests", "Firmware"]:
                copy(input_path, wfilename, extract_dir, start, length, checksums)
            else:
                decryptfile(
                    key,
                    iv,
    ...
    print(f"\nDone. Extracted files to {extract_dir}")


def cli_main(argv=sys.argv):
    if len(argv) != 3:
        print("Oppo Qualcomm decrypt tool v1.1 (c) B.Kerler 2020-2022\n")
        print("Usage: ofp_qc_decrypt <input.ofp> <output directory>")
        return 1

    input_path = argv[1]
    extract_dir = argv[2]

    main(input_path, extract_dir)
