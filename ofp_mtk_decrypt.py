...
def main(input_path, extract_dir):
    ...
    with input_path.open("rb") as rf:
        ...
        for i in range(0, len(hdr2) // 0x60):
            ...
            print(f'Writing "{name}" as "{extract_dir}/{filename}"...')
            with (extract_dir / filename).open("wb") as wb:
                if enclength > 0:
                    rf.seek(start)
    ...
    print(f"Files successfully decrypted to subdirectory {extract_dir}")


def cli_main(argv=sys.argv):
    if len(argv) != 3:
        print("Oppo MediaTek OFP decrypt tool v1.1 (c) B.Kerler 2020-2022\n")
        print("Usage: ofp_mtk_decrypt <input.ofp> <output directory>")
        return 1
